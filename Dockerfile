...
# Install third party software and bash-completion (Canonical) for git CLI completion
...
RUN --mount=type=secret,id=proxy_cert,required=false \
    ...
    && apt-get update \
    && apt-get -y upgrade \
    && apt-get install -y --no-install-recommends gnupg lsb-release wget bash-completion \
