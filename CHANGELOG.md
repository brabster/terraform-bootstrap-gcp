...
## [Unreleased] - Fix duplicate security vulnerability alerts

### Fixed

- Resolved duplicate vulnerability alerts in GitHub security tab by adding SARIF deduplication step.
- OSV Scanner generates SARIF files without proper fingerprints, causing GitHub Code Scanning to display duplicate alerts (78 shown instead of 46 unique vulnerabilities).
- New script `scripts/deduplicate_sarif.py` post-processes SARIF output to generate stable fingerprints and remove duplicate entries.
- Updated CI/CD workflow to apply deduplication before uploading scan results to Code Scanning.

### Technical Details

- The issue affects osv-scanner version 2.2.4, which produces SARIF results without the `partialFingerprints` field that GitHub Code Scanning uses for deduplication.
- Fingerprints are generated by hashing the combination of CVE identifier, package name, package version, and location URI.
- This ensures each unique vulnerability is reported only once while preserving all relevant security information.
