...
# Upgrade pip and setuptools to specific versions to mitigate vulnerabilities
...
# Versions are pinned to reduce supply-chain risk during image build
python3 -m pip install --no-cache-dir --break-system-packages --ignore-installed pip==25.3 setuptools==80.9.0
